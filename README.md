...
SpikeNet-X is a new model variant that uses temporal attention. You can run it by specifying `--model spikenetx`.

### Basic Training

This command starts a standard training run from scratch

```bash
# DBLP Example
python main.py --model spikenetx --dataset dblp --hids 64 --sizes 2 --epochs 100 --lr 0.005 --heads 4 --topk 8 --W 8
```

### Training Workflow (with Checkpoints)

The training script now supports saving, resuming, and testing.

**1. Standard Training & Saving**

When you run a standard training, the script will automatically save the model with the best validation performance to the `checkpoints/` directory (or the directory specified by `--checkpoint_dir`).

```bash
# The best model will be saved as checkpoints/best_model_DBLP.pth
python main.py --model spikenetx --dataset DBLP --epochs 100
```

**2. Resuming Training**

If your training was interrupted, you can resume from the last saved checkpoint using the `--resume_path` argument.

```bash
# This will load the model, optimizer, and epoch number from the checkpoint and continue training.
python main.py --model spikenetx --dataset DBLP --epochs 100 --resume_path checkpoints/best_model_DBLP.pth
```

**3. Testing a Model**

To evaluate a trained model on the test set without running the full training loop, use the `--test_model_path` argument.

```bash
# This will load the model, run evaluation on the test set, and print the results.
python main.py --model spikenetx --dataset DBLP --test_model_path checkpoints/best_model_DBLP.pth
```
